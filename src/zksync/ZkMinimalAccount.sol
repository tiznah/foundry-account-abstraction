...
contract ZkMinimalAccount is IAccount, Ownable {
    ...
    error ZkMinimalAccount__InsufficientBalance(uint256 required, uint256 available);
    ...
    receive() external payable {
        // recieves funds if ETH is sent directly to the contract
    }
    ...
    // Blank since we don't use a paymaster
    ...
    function _validateTransaction(Transaction memory _transaction) internal returns (bytes4 magic) {
        ...
        if (totalRequiredBalance > address(this).balance) {
            revert ZkMinimalAccount__InsufficientBalance(totalRequiredBalance, address(this).balance);
        }
